...
- `!` **không sinh ra code JS nào cả** lúc runtime (biến mất hoàn toàn khi compile) — nó chỉ là lời "nói dối" với TypeScript lúc biên dịch: *"tin tôi đi, giá trị này chắc chắn không null, đừng báo lỗi type nữa"*. Nếu nói dối sai, runtime vẫn chạy y như không có `!`, nhưng compiler đã bị tắt tiếng cảnh báo đúng ngay chỗ có thể cứu bạn sau này.
...
- Quy tắc: hỏi "nếu người gọi không truyền, có 1 giá trị mặc định *hợp lý về mặt ý nghĩa* không?" Có → optional. Không → bắt buộc.
...
- Bước tiếp theo: đặt `console.log` ngay trước dòng lỗi để in ra giá trị thật, so sánh với những gì mình *tưởng* nó sẽ là — đặc biệt quan trọng khi làm việc với API có response shape lồng nhau (VD TVmaze search trả `{ score, show }`, không phải `show` phẳng).
